"""Downstream benchmark consumers for crawler recipes."""
...
def _benchmark_spec(recipe: CrawlerRecipe, expected_kind: str | None = None):
    ...
    if expected_kind is not None and spec.kind != expected_kind:
        raise ValueError(f"Recipe {recipe.name!r} is wired for {spec.kind!r}, not {expected_kind!r}.")
    return spec
    ...
    def run(
        ...
        *,
        ...
        if not isinstance(env_name, str) or not env_name:
            raise ValueError(f"Recipe {recipe.name!r} does not define an RL benchmark env_name.")
        merged_override = _normalize_rl_config_override(spec.config_override)
        merged_override.update(_normalize_rl_config_override(self.default_config_override))
        if config_override:
...
class ProbeConditionedPPO:
    ...
    def _run_probe_pipeline(
        ...
        *,
        ...
        from .app.ppo import run_probe_conditioned_pipeline
...
@dataclass(frozen=True)
class BoardProbeBenchmarkConsumer:
    """Compatibility consumer for the exact board-game rule benchmark."""

    def run(self, recipe: CrawlerRecipe, **_unused):
        config = _benchmark_spec(recipe, expected_kind="board_probe").config
        return run_board_probe_benchmark(config)
