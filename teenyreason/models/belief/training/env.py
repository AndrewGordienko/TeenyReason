...
from ..core.common import sanitize_tensor
...
def run_env_belief_update(
    *,
    ...
    uncapped_env_loss_terms = dict(env_loss_terms)
    repair_cap_exempt_terms = {
        "env_contrastive",
        "env_retrieval",
        "raw_env_retrieval",
    ...
    env_loss_terms = cap_primary_env_loss_terms(
        env_loss_terms,
        max_term_fraction=0.35 if representation_repair_mode else 0.15,
        exempt_names=repair_cap_exempt_terms if representation_repair_mode else (),
    )
    ...
    if representation_repair_mode:
        core_env_loss = sanitize_tensor(
            ...
            + env_loss_terms["env_leaveout_future"]
            + env_loss_terms["env_leaveout_consistency"]
            + env_loss_terms["env_contrastive"]
            + env_loss_terms["env_retrieval"]
            + env_loss_terms["raw_env_retrieval"]
            + env_loss_terms["env_retrieval_margin"]
            + env_loss_terms["raw_env_retrieval_margin"]
            + env_loss_terms["env_gap_ratio"]
    ...
    return {
        ...
        "env_representation_repair_mode": float(1.0 if representation_repair_mode else 0.0),
        ...
        "env_uncapped_retrieval_term": float(uncapped_env_loss_terms["env_retrieval"].item()),
        "env_capped_retrieval_term": float(env_loss_terms["env_retrieval"].item()),
        "env_uncapped_raw_retrieval_term": float(uncapped_env_loss_terms["raw_env_retrieval"].item()),
        "env_capped_raw_retrieval_term": float(env_loss_terms["raw_env_retrieval"].item()),
        "env_uncapped_spread_term": float(uncapped_env_loss_terms["env_spread"].item()),
        "env_capped_spread_term": float(env_loss_terms["env_spread"].item()),
