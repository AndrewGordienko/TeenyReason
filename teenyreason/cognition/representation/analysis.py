...
def _support_family_summary(
    support_mask: torch.Tensor,
    view_group_id: np.ndarray,
    probe_mode_names: np.ndarray,
) -> dict[str, np.ndarray]:
    """Summarize support-family coverage without inventing a winner on ties."""
    support_np = support_mask.cpu().numpy().astype(np.float32)
    family_names = np.asarray(probe_mode_names, dtype="U")
    row_count = support_np.shape[0]
    dominant_modes = np.full((row_count,), "none", dtype="U32")
    top_share = np.zeros((row_count,), dtype=np.float32)
    effective_count = np.zeros((row_count,), dtype=np.float32)
    entropy = np.zeros((row_count,), dtype=np.float32)
    tied_top_count = np.zeros((row_count,), dtype=np.float32)

    for env_idx in range(row_count):
        support_idx = np.flatnonzero(support_np[env_idx] > 0)
        if support_idx.size == 0:
            continue
        support_groups = np.asarray(view_group_id[env_idx, support_idx], dtype=np.int32)
        support_groups = support_groups[support_groups >= 0]
        if support_groups.size == 0:
            continue
        unique_groups, group_counts = np.unique(support_groups, return_counts=True)
        shares = group_counts.astype(np.float32) / float(max(int(group_counts.sum()), 1))
        max_count = int(np.max(group_counts))
        tied_groups = unique_groups[group_counts == max_count]
        top_share[env_idx] = float(np.max(shares))
        effective_count[env_idx] = float(1.0 / max(float(np.sum(np.square(shares))), 1e-6))
        entropy[env_idx] = float(-np.sum(shares * np.log(np.clip(shares, 1e-8, None))))
        tied_top_count[env_idx] = float(tied_groups.size)
        if tied_groups.size == 1:
            group_idx = int(np.clip(tied_groups[0], 0, max(family_names.size - 1, 0)))
            dominant_modes[env_idx] = str(family_names[group_idx])
        else:
            dominant_modes[env_idx] = "mixed"
...
def build_env_belief_dataset(
    ...
    support_family_summary = _support_family_summary(
        support_mask=support_mask_t,
        view_group_id=grouped["view_group_id"],
        probe_mode_names=probe_mode_names,
    )
    env_dominant_probe_mode = support_family_summary["dominant_mode"]
    ...
    return {
        ...
        "env_support_top_family_share": support_family_summary["top_share"].astype(np.float32),
        "env_support_effective_family_count": support_family_summary["effective_count"].astype(np.float32),
        "env_support_family_entropy": support_family_summary["entropy"].astype(np.float32),
        "env_support_tied_top_family_count": support_family_summary["tied_top_count"].astype(np.float32),
