...
from .benchmark_support import (
    aggregate_matched_eval_summary,
...
def finalize_benchmark_run(
    *,
    ...
    latent_win_gate = evaluate_latent_win_gate(
        ...
        latent_split_retrieval=latent_split_top1,
    ...
    print_solve_summary(
        "Full-system controller solve episode",
        full_system_episode_solves,
        full_system_completed_episodes,
    )
    print_solve_summary(
        "Full-system controller solve env steps",
        full_system_step_solves,
        full_system_total_env_steps,
    )
    print_matched_eval_summary(
        "Full-system controller matched learned eval",
        aggregate_matched_eval_summary(results, "full_system_learned_eval_summary"),
    )
    print_matched_eval_summary(
        "Full-system controller matched state-only eval",
        aggregate_matched_eval_summary(results, "full_system_state_only_eval_summary"),
    )
    print_matched_eval_summary(
        "Full-system controller matched zero-context eval",
        aggregate_matched_eval_summary(results, "full_system_zero_context_eval_summary"),
    )
    print_matched_eval_summary(
        "Full-system controller matched shuffled-context eval",
        aggregate_matched_eval_summary(results, "full_system_shuffled_context_eval_summary"),
    )
    print_matched_eval_summary(
        "Full-system controller matched stale-context eval",
        aggregate_matched_eval_summary(results, "full_system_stale_context_eval_summary"),
    )
    print_matched_eval_summary(
        "Full-system controller matched no-refresh eval",
        aggregate_matched_eval_summary(results, "full_system_online_refinement_eval_summary"),
    )
    print_matched_eval_summary(
        "Full-system controller matched frozen-context eval",
        aggregate_matched_eval_summary(results, "full_system_frozen_context_eval_summary"),
    )
    print_solve_summary(
        "Full-system oracle controller solve episode",
        full_system_oracle_episode_solves,
        full_system_oracle_completed_episodes,
    )
    print_solve_summary(
        "Full-system oracle controller solve env steps",
        full_system_oracle_step_solves,
        full_system_oracle_total_env_steps,
    )
    print_solve_summary(
        "Sim-fanout solve episode",
        sim_fanout_episode_solves,
        sim_fanout_completed_episodes,
    )
    print_solve_summary(
        "Sim-fanout solve env steps",
