...
from teenyreason.crawler import EnvExpression, MetricBelief, PredictiveBelief, UncertaintyEstimate
...
from teenyreason.rl.probe_policy.budgeting import (
...
from teenyreason.rl.probe_policy.handoff.diagnostics import (
...
from teenyreason.rl.probe_policy.training.reporting import (
...
from teenyreason.crawler.probes.latent import probe_group_ids_from_families
...
class ProbeLogicTests(unittest.TestCase):
    ...
    def test_strict_gate_names_particle_subset_blocker(self):
        expression = EnvExpression(
            vector=np.asarray([0.1, 0.2], dtype=np.float32),
            confidence=0.9,
            ready=True,
            uncertainty_scalar=0.2,
            compressed=False,
            metadata={
                "belief_source": "sysid",
                "message_mode": "on",
                "readiness_score": 0.8,
                "future_probe_quality": 0.9,
                "online_subset_stability": 0.1,
                "leaveout_stability": 0.9,
                "online_offline_gap": 0.0,
                "online_geometry_complete": True,
            },
        )

        diagnostics = fair_env_expression_diagnostics(env_expression=expression)

        self.assertFalse(diagnostics["enabled"])
        self.assertEqual(diagnostics["blocker"], "particle_subset_stability")
    ...
    def test_probe_run_requires_context_identity_before_latent_win_label(self):
        classification = classify_probe_run(
            baseline_episode=120,
            baseline_steps=9000,
            probe_episode=80,
            ...
            probe_env_expression_delta=12.0,
            probe_fair_ready_handoff_fraction=0.4,
            probe_fair_expression_enabled_fraction=0.25,
            full_system_zero_context_ablation_delta=0.0,
            full_system_shuffled_context_ablation_delta=0.0,
            full_system_stale_context_ablation_delta=0.0,
        )

        self.assertEqual(classification, "protocol_win")

    def test_probe_run_requires_full_benchmark_before_latent_win_label(self):
        classification = classify_probe_run(
            baseline_episode=120,
            baseline_steps=9000,
            probe_episode=80,
            ...
            probe_fair_ready_handoff_fraction=0.4,
            probe_fair_expression_enabled_fraction=0.25,
            full_system_zero_context_ablation_delta=20.0,
            full_system_shuffled_context_ablation_delta=20.0,
            full_system_stale_context_ablation_delta=20.0,
    ...
    def test_fair_probe_second_family_stops_when_no_family_clears_economics(self):
        chosen = choose_fair_probe_family(
            family_names=("passive_decay", "chirp", "impulse_left"),
            expected_family_gain={
                "chirp": {
                    ...
                    "value_per_probe_step": 0.03,
                    "selection_score": 0.04,
                    "estimated_probe_cost": 1.0,
                },
                "impulse_left": {
                    "future_gain_for_choice": 0.35,
                    "predicted_marginal_value": 0.05,
    ...
    def test_fair_probe_second_family_prefers_sample_efficiency_over_raw_gain(self):
        chosen = choose_fair_probe_family(
            family_names=("passive_decay", "expensive_high_gain", "cheap_control_probe"),
            expected_family_gain={
                "expensive_high_gain": {
                    "future_gain_for_choice": 0.90,
                    "predicted_split_reduction": 0.10,
                    "predicted_mechanics_reduction": 0.10,
                    "predicted_marginal_value": 0.45,
                    "value_per_probe_step": 0.08,
                    "selection_score": 0.45,
                    "estimated_probe_cost": 5.0,
                },
                "cheap_control_probe": {
                    "future_gain_for_choice": 0.40,
                    "predicted_split_reduction": 0.16,
                    "predicted_mechanics_reduction": 0.14,
                    "predicted_marginal_value": 0.30,
                    "value_per_probe_step": 0.30,
                    "selection_score": 0.30,
                    "estimated_probe_cost": 1.0,
            ...
            family_counts={
                "passive_decay": 0,
                "expensive_high_gain": 0,
                "cheap_control_probe": 0,
            },
            probe_count=1,
        )

        self.assertEqual(chosen, "cheap_control_probe")
